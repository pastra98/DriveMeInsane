...
func show_main_menu():
    ...
    var bg_scene = load("res://environment/menu_background/MenuBackground.tscn").instantiate()
    main.add_child(bg_scene)
    var mm = load("res://gui/main_menu/MainMenu.tscn").instantiate()
    mm.get_node("LevelPicker").connect("new_level_picked", Callable(main, "load_level"))
    add_child(mm)
...
func show_passenger_picker(passenger_refs_arr: Array, lvl_info: Dictionary):
    clear_control_children()
    var before_level = load("res://gui/before_level/BeforeLevel.tscn").instantiate()
    ...
    before_level.get_node("PassengerPicker").connect("level_started", Callable($"/root/Main/Level", "start_level"))
...
func show_game_hud(player: Node2D, passenger_refs_arr: Array):
    ...
    clear_control_children()
    var hud = load("res://gui/game_hud/GameHUD.tscn").instantiate()
    ...
    player.connect("new_health", Callable($"GameHUD/PlayerStatus", "update_health"))
    player.connect("new_score", Callable($"GameHUD/PlayerStatus", "update_score"))
    # add the passenger windows
    for passenger_ref in passenger_refs_arr:
        var new_passenger_window = load("res://gui/game_hud/passenger_window/PassengerWindow.tscn").instantiate()
        passenger_ref.connect("new_insanity", Callable(new_passenger_window, "update_insanity"))
        passenger_ref.connect("new_picture", Callable(new_passenger_window, "update_picture"))
    ...
    if not main.is_tutorial_completed:
        ...
        await hud.get_node("BeforeStartTutorial").confirmed
...
func show_level_over_gui(lvl_nr: int, stars: int, points: int):
    ...
    if n_unlock_pass < main.unlocked_passengers.size():
        ...
        var unlocker = load("res://gui/game_over/passenger_unlocks/PassengerUnlocks.tscn").instantiate()
        ...
        await close_button.button_down
        unlocker.queue_free()
    # then show the game over screen
    var game_over_screen = load("res://gui/game_over/GameOver.tscn").instantiate()
