...
func add_passenger_cards(passenger_instances: Array):
    ...
    for passenger in passenger_instances:
        ...
        connect("all_passengers_seated", Callable(new_card, "_on_all_passengers_seated"))
        connect("free_seats_available", Callable(new_card, "_on_free_seats_available"))
        new_card.connect("passenger_added_to_player", Callable(self, "seat_passenger"))
        new_card.connect("passenger_added_to_player", Callable(level, "add_passenger_to_player"))
    ...
    for seat in seats.get_children():
        seat.connect("passenger_removed_from_player", Callable(self, "unseat_passenger"))
        seat.connect("passenger_removed_from_player", Callable(level, "remove_passenger_from_player"))
